	...
	*/
	$.fn.slotMachine = function(settings){
		...
		*/ 
		...
		function _getDiy(rnd){
			var rnd = (rnd||0)-1<0 ? $titles.length-1 : rnd-1;
			var prevObj = {
				index	: rnd,
				el		: $titles.get(rnd)
			};
		...
		*/
		...
		function _stopDiy(nowOrReations,diyFnIndex){
			if(typeof diyFnIndex=="function"){
				_stop(nowOrReations,function(){return _getDiy(diyFnIndex());});
			}else{
				_stop(nowOrReations,function(){return _getDiy(diyFnIndex);});
			}
		}
		...
		*/
		...
		$slot.stopDiy=function(nowOrRepeations,fnIndex){
			_forceStop = true;
			...
			_stopDiy(nowOrRepeations,fnIndex);
		};
